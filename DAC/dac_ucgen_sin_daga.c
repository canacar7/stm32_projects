...
uint16_t sinArray[64] = {2048,2248,2447,2642,2831,3013,3185,3346,3495,3630,3750,3853,
...
uint16_t ucgenArray[64] = {128,256,384,512,640,768,896,1024,
...
void GPIO_config(){
	...
	GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_AIN; //gurultuleri engellemk icin ain girisini seciyourum
...
while(1){
	...
	for(int i = 0; i < 64; i++){
		DAC_SetChannel1Data(DAC_Align_12b_R,sinArray[i]);     //sin dalgasi
		//DAC_SetChannel1Data(DAC_Align_12b_R,ucgenArray[i]); //ucgen dalgasi
		delay(360000);
